$(function () {
    "use strict";
    if ($('.audio-player').length) {
        var myPlayListOption = '<ul class="more_option"><li><a href="#"><span class="opt_icon" title="Add To Favourites"><span class="icon icon_fav"></span></span></a></li><li><a href="#"><span class="opt_icon" title="Add To Queue"><span class="icon icon_queue"></span></span></a></li><li><a href="#"><span class="opt_icon" title="Download Now"><span class="icon icon_dwn"></span></span></a></li><li><a href="#"><span class="opt_icon" title="Add To Playlist"><span class="icon icon_playlst"></span></span></a></li><li><a href="#"><span class="opt_icon" title="Share"><span class="icon icon_share"></span></span></a></li></ul>';

        var myPlaylist = new jPlayerPlaylist({
            jPlayer: "#jquery_jplayer_1",
            cssSelectorAncestor: "#jp_container_1"
        }, [], {
            swfPath: "js/plugins",
            supplied: "mp3, wav",
            wmode: "window",
            useStateClassSkin: true,
            autoBlur: false,
            smoothPlayBar: true,
            keyEnabled: true,
            repeat: true,
            playlistOptions: {
                autoPlay: true
            }
        });
        $("#jquery_jplayer_1").on($.jPlayer.event.ready + ' ' + $.jPlayer.event.play, function (event) {
            var current = myPlaylist.current;
            var playlist = myPlaylist.playlist;
            $.each(playlist, function (index, obj) {
                if (index == current) {
                    $(".jp-now-playing").html("<div class='jp-track-name'><span class='que_img'><img src='/static/" + obj.image + "'></span><div class='que_data'>" + obj.title + " <div class='jp-artist-name'>" + obj.artist + "</div></div></div>");
                }
            });
            $('.knob-wrapper').mousedown(function () {
                $(window).mousemove(function (e) {
                    var angle1 = getRotationDegrees($('.knob')),
                        volume = angle1 / 270

                    if (volume > 1) {
                        $("#jquery_jplayer_1").jPlayer("volume", 1);
                    } else if (volume <= 0) {
                        $("#jquery_jplayer_1").jPlayer("mute");
                    } else {
                        $("#jquery_jplayer_1").jPlayer("volume", volume);
                        $("#jquery_jplayer_1").jPlayer("unmute");
                    }
                });

                return false;
            }).mouseup(function () {
                $(window).unbind("mousemove");
            });


            function getRotationDegrees(obj) {
                var matrix = obj.css("-webkit-transform") ||
                    obj.css("-moz-transform") ||
                    obj.css("-ms-transform") ||
                    obj.css("-o-transform") ||
                    obj.css("transform");
                if (matrix !== 'none') {
                    var values = matrix.split('(')[1].split(')')[0].split(',');
                    var a = values[0];
                    var b = values[1];
                    var angle = Math.round(Math.atan2(b, a) * (180 / Math.PI));
                } else {
                    var angle = 0;
                }
                return (angle < 0) ? angle + 360 : angle;
            }

            var timeDrag = false;
            $('.jp-play-bar').mousedown(function (e) {
                timeDrag = true;
                updatebar(e.pageX);

            });
            $(document).mouseup(function (e) {
                if (timeDrag) {
                    timeDrag = false;
                    updatebar(e.pageX);
                }
            });
            $(document).mousemove(function (e) {
                if (timeDrag) {
                    updatebar(e.pageX);
                }
            });
            var updatebar = function (x) {
                var progress = $('.jp-progress');
                var position = x - progress.offset().left;
                var percentage = 100 * position / progress.width();
                if (percentage > 100) {
                    percentage = 100;
                }
                if (percentage < 0) {
                    percentage = 0;
                }
                $("#jquery_jplayer_1").jPlayer("playHead", percentage);
                $('.jp-play-bar').css('width', percentage + '%');
            };
            $('#playlist-toggle, #playlist-text, #playlist-wrap li a').unbind().on('click', function () {
                $('#playlist-wrap').fadeToggle();
                $('#playlist-toggle, #playlist-text').toggleClass('playlist-is-visible');
            });
            $('.hide_player').unbind().on('click', function () {
                $('.audio-player').toggleClass('is_hidden');
                $(this).html($(this).html() == '<i class="fa fa-angle-down"></i> HIDE' ? '<i class="fa fa-angle-up"></i> SHOW PLAYER' : '<i class="fa fa-angle-down"></i> HIDE');
            });
            $('body').unbind().on('click', '.audio-play-btn', function () {
                $('.audio-play-btn').removeClass('is_playing');
                $(this).addClass('is_playing');
                var playlistId = $(this).data('playlist-id');
            });

        });

        $('.ms_play_icon.playing').on("click", function () {
            $.ajax({
                type: "POST",
                url: "/add_history",
                data: {
                    music_id: $(this).attr("music_id")
                }
            })
            
            let id = $(this).attr('music_id');
            let musics = $(this).attr('musics');
            let array = musics.replace(/\)\,/g, ' ').replace(/[[\]]/g, "").replace(/[()']/g, "").split("  ");
            let a = [];
            let counter = 0;
            for (let i = 0; i <= array.length; i++) {
                counter++;
                if (array[i].indexOf(id) != -1) {
                    break;
                }
            }

            if (myPlaylist.playlist.length > 0) {
                myPlaylist.current = counter - 1;
                myPlaylist.play(myPlaylist.current);
            } else {
                for (let i = 0; i <= array.length; i++) {
                    a = array[i].split(", ");
                    console.log(a[i])
                    myPlaylist.add({
                        image: a[4],
                        title: a[1],
                        artist: a[3],
                        mp3: a[2]
                    }, true);
                    myPlaylist.current = counter - 1;
                    myPlaylist.play(myPlaylist.current);
                }
            }
            $('.jp-shuffle').on("click", function() {
                myPlaylist.shuffle(true);
                console.log(myPlaylist.shuffle())
            })
            $('.jp-repeat').on("click", function() {
                myPlaylist.repeat(true);
                console.log(myPlaylist.repeat());
            })
        });
        $('.clear_all').click(function () {
            myPlaylist.remove();
        });
        
        var audio = $('audio')[0];
        let timeout_id = 0;

        $(audio).on('play', function() {
            timeout_id = window.setTimeout(function () {
                $.ajax({
                    type: "POST",
                    url: "/stream",
                    data: {
                        music_title: audio.title
                    },
                    success: function (resp) {
                        console.log("success")
                    },
                    error: function (error) {
                        console.log(error)
                    }
                })
            }, 20000);
        }).on('emptied', function() {
            console.log("stopped playing");
            window.clearInterval(timeout_id);
        })

        var lastSecond = 0;
        var secondsToCallFunction = 20;
        $(audio).on('timeupdate', function (e) {
            var seconds = Math.floor(e.target.currentTime);
            if (seconds % secondsToCallFunction == 0 && lastSecond !== seconds) {

            }
        });
    }
});

// for (let i = 0; i <= array.length; i++) {
//     a = array[i].split(", ");
//     playlist.push({    
//         image: a[4],
//         title: a[1],
//         artist: a[3],
//         mp3: a[2]
//     });
//     myPlaylist.setPlaylist(playlist);
//     myPlaylist.current = counter - 1;
// };


// var au = document.createElement('audio');
// au.src = "https://kajohny.pythonanywhere.com/static/music/16 кыз.mp3";
// au.addEventListener('loadedmetadata', function () {
//     var minutes = Math.floor(au.duration / 60);
//     var duration = minutes + ":" + Math.floor(au.duration - minutes * 60); 
//     console.log(duration)
// }, false);


// $(audio).on('playing', function(event) {
//     timeout_id = window.setInterval(function() {
//         console.log("nigga")
//     }, 5000);
// }).on('pause', function(event) {
//     console.log("nigga dead")
//     window.clearInterval(timeout_id)
// });

// $(audio).on('timeupdate', function (event) {
//     timeout_id = window.setInterval(function () {
//         console.log("LOX");
//     }, 20000);
// }).on('emptied', function (event) {
//     console.log("JOPA");
//     window.clearInterval(timeout_id);
// })